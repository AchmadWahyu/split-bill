...
import mockData from '../../src/mocks/localStorage/all-payer/all-payer-multiple-person-with-tax-service-charge.json' with { type: 'json' };

test.describe('All Person is Payer - Multiple Person - With Tax & Service Charge', () => {
  ...
  test('should display event on home page and show correct details', async ({
    ...
    const eventTitle = page.getByText('All Person is Payer - Multiple Person - With Tax & Service Charge');
    await expect(eventTitle).toBeVisible();

    // Verify the event shows 3 people
    await expect(page.getByText('3 orang')).toBeVisible();

    // Verify total expense is displayed (270000 + 5000 + 10% tax = 302500)
    // Format: Rp 302.500 (Indonesian format)
    await expect(page.getByText(/Rp\s*302[.,]500/i)).toBeVisible();
    ...
    await expect(page).toHaveURL(
      new RegExp('http://localhost:5173/acara/event-all-payer-with-tax-service-charge/?$')
    );

    // Verify the event title is displayed
    await expect(
      page.getByRole('heading', {
        name: 'All Person is Payer - Multiple Person - With Tax & Service Charge',
      })
    ).toBeVisible();
