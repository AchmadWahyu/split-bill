...
import mockData from '../../src/mocks/localStorage/all-payer/all-payer-multiple-person-with-discount-service-charge.json' with { type: 'json' };

test.describe('All Person is Payer - Multiple Person - With Discount & Service Charge', () => {
  ...
  test('should display event on home page and show correct details', async ({
    ...
    const eventTitle = page.getByText('All Person is Payer - Multiple Person - With Discount & Service Charge');
    await expect(eventTitle).toBeVisible();

    // Verify the event shows 3 people
    await expect(page.getByText('3 orang')).toBeVisible();

    // Verify total expense is displayed (270000 - 20000 + 5000 = 255000)
    // Format: Rp 255.000 (Indonesian format)
    await expect(page.getByText(/Rp\s*255[.,]000/i)).toBeVisible();
    ...
    await expect(page).toHaveURL(
      new RegExp('http://localhost:5173/acara/event-all-payer-with-discount-service-charge/?$')
    );

    // Verify the event title is displayed
    await expect(
      page.getByRole('heading', {
        name: 'All Person is Payer - Multiple Person - With Discount & Service Charge',
      })
    ).toBeVisible();
