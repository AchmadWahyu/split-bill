...
import mockData from '../../src/mocks/localStorage/double-payer/double-payer-multiple-person-with-tax.json' with { type: 'json' };

test.describe('Double Payer - Multiple Person - With Tax', () => {
  ...
  test('should display event on home page and show correct details', async ({
    ...
    const eventTitle = page.getByText('Double Payer - Multiple Person - With Tax');
    await expect(eventTitle).toBeVisible();

    // Verify the event shows 3 people
    await expect(page.getByText('3 orang')).toBeVisible();

    // Verify total expense is displayed (240000 + 10% tax = 264000)
    // Format: Rp 264.000 (Indonesian format)
    await expect(page.getByText(/Rp\s*264[.,]000/i)).toBeVisible();
    ...
    await expect(page).toHaveURL(
      new RegExp('http://localhost:5173/acara/event-double-payer-with-tax/?$')
    );

    // Verify the event title is displayed
    await expect(
      page.getByRole('heading', {
        name: 'Double Payer - Multiple Person - With Tax',
      })
    ).toBeVisible();
