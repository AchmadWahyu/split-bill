...
import mockData from '../../src/mocks/localStorage/double-payer/double-payer-multiple-person-no-discount-tax-service.json' with { type: 'json' };

test.describe('Double Payer - Multiple Person - No Discount/Tax/Service', () => {
  ...
  test('should display event on home page and show correct details', async ({
    ...
    const eventTitle = page.getByText('Double Payer - Multiple Person - No Discount/Tax/Service');
    ...
    await expect(page).toHaveURL(
      new RegExp('http://localhost:5173/acara/event-double-payer-no-discount-tax-service/?$')
    );

    // Verify the event title is displayed
    await expect(
      page.getByRole('heading', {
        name: 'Double Payer - Multiple Person - No Discount/Tax/Service',
      })
    ).toBeVisible();
