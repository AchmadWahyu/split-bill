...
import mockData from '../../src/mocks/localStorage/single-payer/single-payer-multiple-person-no-discount-tax-service.json' with { type: 'json' };

test.describe('Single Payer - Multiple Person - No Discount/Tax/Service', () => {
  ...
  test('should display event on home page and show correct details', async ({
    ...
    const eventTitle = page.getByText('Single Payer - Multiple Person - No Discount/Tax/Service');
    ...
    await expect(page).toHaveURL(
      new RegExp('http://localhost:5173/acara/event-single-payer-no-discount-tax-service/?$')
    );

    // Verify the event title is displayed
    await expect(
      page.getByRole('heading', {
        name: 'Single Payer - Multiple Person - No Discount/Tax/Service',
      })
    ).toBeVisible();
