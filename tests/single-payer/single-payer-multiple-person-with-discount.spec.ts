...
import mockData from '../../src/mocks/localStorage/single-payer/single-payer-multiple-person-with-discount.json' with { type: 'json' };

test.describe('Single Payer - Multiple Person - With Discount', () => {
  ...
  test('should display event on home page and show correct details', async ({
    ...
    const eventTitle = page.getByText('Single Payer - Multiple Person - With Discount');
    await expect(eventTitle).toBeVisible();

    // Verify the event shows 3 people
    await expect(page.getByText('3 orang')).toBeVisible();

    // Verify total expense is displayed (210000 - 10000 = 200000)
    // Format: Rp 200.000 (Indonesian format)
    await expect(page.getByText(/Rp\s*200[.,]000/i)).toBeVisible();
    ...
    await expect(page).toHaveURL(
      new RegExp('http://localhost:5173/acara/event-single-payer-with-discount/?$')
    );

    // Verify the event title is displayed
    await expect(
      page.getByRole('heading', {
        name: 'Single Payer - Multiple Person - With Discount',
      })
    ).toBeVisible();
